...
{
    public KeyframedAnimation() : this([])
    {
    }

    public KeyframedAnimation(Keyframe[] positions) : base((int)
        Math.Ceiling(positions.Sum(position => position.LengthMs)))
    {
        Positions = positions;
    ...
    public KeyframedAnimation(Keyframe[] positions, Action callbackOnFinish) : this(positions)
    {
        CallbackOnFinish = callbackOnFinish;
    }

    public Keyframe[] Positions { get; }
    ...
    private (Keyframe Current, Keyframe? Next, float Progress) GetCurrentState()
    {
        ...
        {
            if (Positions[i].LoopingMode == AnimationLoopingMode.LoopStart)
            {
                loopStartIndex = i;
                loopStartMs = currentMs;
            }

            if (Positions[i].LoopingMode == AnimationLoopingMode.Invert)
            {
                invertIndex = i;
                invertMs = currentMs;
            }

            currentMs += Positions[i].LengthMs;
        }
        ...
        else if (totalLength > 0)
        {
            switch (LoopingMode)
            {
                ...
                default:
                    if (elapsed >= totalLength)
                    {
                        if (Positions.Length == 0) return (default, null, 0);
                        if (!TimingStopwatch.IsRunning) return (Positions[^1], null, 1f);

                        TimingStopwatch.Stop();
                        CallbackOnFinish?.Invoke();
                        return (Positions[^1], null, 1f);
                    }
        ...
        for (var i = 0; i < Positions.Length; i++)
        {
            ...
            if (elapsed < currentStart + data.LengthMs)
            {
                ...
                return (data, i + 1 < Positions.Length ? Positions[i + 1] : null, progress);
            }
