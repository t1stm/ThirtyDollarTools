...
public sealed class ThirtyDollarApplication : Scene, IGamePreloadable
{
    ...
    public ThirtyDollarApplication(Game game, int width, int height, string?[] sequenceLocations,
        ...
        ThirtyDollarWorkflow workflow) : base(game.SceneManager)
    {
        _workflow = workflow;
    ...
    public override void Initialize(InitArguments initArguments)
    {
        ...
        _workflow.OnLoaded = async () =>
        {
            try
            {
                if (_startingSequences.Length < 1) return;
                await _workflow.UpdateSequences(_startingSequences);
            }
            catch (Exception e)
            {
        ...
        _workflow.Log = str => SetStatusMessage(str, 3500);

        _loadingAsyncTask = _workflow.Game.ThreadRunner.RunTask(() =>
        {
            _workflow.GetSampleHolder().GetAwaiter().GetResult();
        ...
        switch (left_control)
        {
            ...
            case true:
            {
                ...
                if (state.IsKeyPressed(Keys.D))
                {
                    ...
                    _applicationTextContainer.ShowDebug = _workflow.ShowDebugInfo;
    ...
    {
        if (!ShowDebugInfo || _workflow.SampleHolder is null) return;
        if (_loadingAsyncTask is not { IsCompleted: true }) return;
        ...
        if (current_index < placement_length)
        {
            ...
            for (var i = current_index; i < TimedEvents.Placement.Length; i++)
            {
                ...
                if (soundEvent is not null && _workflow.SampleHolder is not null)
                {
