...
///     <para>
///         Allocates nothing: the caller owns both the output span and the
...
internal static class MsdfGenerator
{
    ...
    ///     opens on a cold distance bound, so the band size balances keeping every core busy
    ///     against amortising that first slow pixel over the band's rows.
    /// </summary>
    ...
    ///     <para>
    ///         <paramref name="finderPool" /> supplies one finder per parallel band, each carrying
    ///         an edge-cache array sized to the glyph. The pool belongs to the caller, so nothing
    ///         outlives the font it came from.
    ///     </para>
    /// </summary>
        ...
        Shape shape, in SdfTransformation transformation, ShapeDistanceFinder finder)
    {
        ...
        // the edge cache stays useful across the row boundary. The cache's bound only ever
        // loosens with distance, so the result does not depend on visit order.
        var rightToLeft = false;

        // Row 0 is the BOTTOM of the glyph, so the buffer is vertically flipped from an image's
        // point of view. The engine hands it straight to an OpenGL texture, whose V axis runs
        // bottom-up and flips it back on sampling.
    ...
    ///     <para>
    ///         Pixels are independent: the distance bound each <see cref="ShapeDistanceFinder" />
    ///         carries for its early-out only decides how much work a pixel does, never its value,
    ///         so banded output is bit-identical to serial. The <see cref="Shape" /> is only read
    ///         from here, so the bands share it.
    ///     </para>
    /// </summary>
