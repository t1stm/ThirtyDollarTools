...
public class ThreeDollarWebsite : ThirtyDollarApplication
{
    ...
    public ThreeDollarWebsite(int width, int height, AudioContext? audio_context): base(width, height, null, audio_context)
    {
        Width = width;
        Height = height;
    ...
    public override void Render()
    {
        ...
        foreach (var renderable in objects_3d)
        {
    ...
    public override void Keyboard(KeyboardState state)
    {
        ...
        {
            _camera.Position += _camera.Front * cameraSpeed * 0.05f; // Forward
        }

        if (state.IsKeyDown(Keys.S))
        {
            _camera.Position -= _camera.Front * cameraSpeed * 0.05f; // Backwards
        }
        if (state.IsKeyDown(Keys.A))
        {
            _camera.Position -= _camera.Right * cameraSpeed * 0.05f; // Left
        }
        if (state.IsKeyDown(Keys.D))
        {
            _camera.Position += _camera.Right * cameraSpeed * 0.05f; // Right
        }
        if (state.IsKeyDown(Keys.Space))
        {
            _camera.Position += _camera.Up * cameraSpeed * 0.05f; // Up
        }
        if (state.IsKeyDown(Keys.LeftShift))
        {
            _camera.Position -= _camera.Up * cameraSpeed * 0.05f; // Down
        }
    }
}